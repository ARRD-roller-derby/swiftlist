    ...
    <Listbox value={selected} onChange={handleSelect}>
      <div className="relative ">
        ...
        <Transition
          ...
          leaveTo="opacity-0"
        >
          <Listbox.Options className="absolute mt-1 -top-60 max-h-60 w-full overflow-auto border-teal border bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm z-10">
            {units.map((unit) => (
              <Listbox.Option
                key={unit}
                className={({ active }) =>
